...
public class ClassBuilder
{
    ...
    public void Build()
    {
        ...
        foreach (var xmlFile in xmlFiles){
            ...
            if (!string.IsNullOrEmpty(className))
            {
                ...
                File.WriteAllText(Path.Combine(outputDir, Path.Combine(packagePath.Replace("sunaba.", "").Replace(".", "/"), $"{className}.hx")), haxeClass);
            }
        }
    }
    ...
    public string GenerateClass(XDocument doc, string className)
    {
        ...
        importList.Add("sunaba.core.nativelink.LinkRef");
        importList.Add("sunaba.core.nativelink.LinkObject");
        ...
        foreach (var _import in importList)
        {
            ...
            if (import.Contains("BinaryData")) import = "sunaba.core.io.BinaryData";
    ...
    {
		sb.AppendLine();
        sb.AppendLine("    public function ready() {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
        sb.AppendLine("    public function enterTree() {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
        sb.AppendLine("    public function exitTree() {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
        sb.AppendLine("    public function process(delta: Float) {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
        sb.AppendLine("    public function physicsProcess(delta: Float) {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
        sb.AppendLine("    public function notification(what: Int) {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
        sb.AppendLine("    public function input(event: InputEvent) {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
        sb.AppendLine("    public function unhandledInput(event: InputEvent) {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
        sb.AppendLine("    public function unhandledKeyInput(event: InputEvent) {");
        sb.AppendLine("        ");
        sb.AppendLine("    }");
                ...
                if (!string.IsNullOrEmpty(memberName))
                {
                    ...
                    if (isVariantType(memberType))
                    {
                        ...
                        if (importName.Contains("BinaryData"))
                            importName = $"sunaba.core.io.{MapReturnType(memberType)}";
                        if (!importList.Contains(importName) && (packageLocations[className] != "sunaba.core" || importName.Contains("BinaryData")))
