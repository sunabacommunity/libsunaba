...
public class ServiceBuilder
{
    ...
    public string GenerateClass(XDocument doc, string className)
    {
        ...
        classSb.AppendLine("        }");
        classSb.AppendLine("        return _native;");
        classSb.AppendLine("    }");
        ...
        foreach (var member in members)
        {
            ...
            if (!MemberExists(memberName, className))
            {
                ...
                if (!string.IsNullOrEmpty(memberName))
                {
                    ...
                    memberSb.AppendLine($"    public static var {newMemberName.ToCamelCase()}(get, set): {MapReturnType(memberType)};");
                    memberSb.AppendLine($"    static function get_{newMemberName.ToCamelCase()}(): {MapReturnType(memberType)} {'{'}");
                    if (isVariantType(memberType))
                    {
                        memberSb.AppendLine($"        return getNative().get('{memberName}');");
                    ...
                    {
                        memberSb.AppendLine($"        var ref: {GetNativeObject(memberType)} = getNative().get('{memberName}');");
                    ...
                    memberSb.AppendLine($"    {'}'}");
                    memberSb.AppendLine($"    static function set_{newMemberName.ToCamelCase()}(value: {MapReturnType(memberType)}): {MapReturnType(memberType)} {'{'}");
                    if (isVariantType(memberType))
                    {
                        memberSb.AppendLine($"      getNative().set('{memberName}', value);");
                    }
                    else if (ClassNames.Contains(memberType))
                    {
                        memberSb.AppendLine($"      getNative().set('{memberName}', value.native);");
                    }
                    else
                    {
                        continue;
    ...
    public void BuildSignals(StringBuilder sb, XDocument doc, string className, List<string> importList)
    {
	    ...
	    foreach (var signal in signals)
	    {
		    ...
		    signalSb.AppendLine($"	private static var _{signalName.ToCamelCase()}: Signal;");
		    signalSb.AppendLine($"	public static var {signalName.ToCamelCase()}(get, default): Signal;");
		    signalSb.AppendLine($"	static function get_{signalName.ToCamelCase()}(): Signal {'{'}");
		    signalSb.AppendLine($"	    if (_{signalName.ToCamelCase()} == null) {'{'}");
		    signalSb.AppendLine($"	        _{signalName.ToCamelCase()} = Signal.{signalMethod}(getNative(), '{signalName}');");
		    signalSb.AppendLine($"	    {'}'}");
            ...
            if (!MethodExists(methodName, className))
            {
                ...
                if (!string.IsNullOrEmpty(methodName))
                {
                    ...
                    else if (isStatic == false)
                    {
                        ...
                        {
                            if (returnType != "void" && returnType != "Void")
                                methodSb.AppendLine($"      return getNative().call('{methodName}', args);");
                            else
                                methodSb.AppendLine($"      getNative().call('{methodName}', args);");
                            var importName = $"sunaba.core.{MapReturnType(returnType)}";
                            if (!importList.Contains(importName) && packageLocations[className] != "sunaba.core")
                                importList.Add(importName);
                        }
                        ...
                        {
                            methodSb.AppendLine($"      var ref: {GetNativeObject(returnType)} = getNative().call('{methodName}', args);");
                            methodSb.AppendLine($"      return new {returnType}(ref);");
                            if (packageLocations[className] != packageLocations[returnType])
                            {
                                var importName = $"{packageLocations[returnType]}.{returnType}";
