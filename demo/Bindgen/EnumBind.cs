	...
	public void GenerateRootEnums()
	{
		...
		foreach (var type in types)
		{
			if (type.IsEnum)
			{
				...
				{
					var valName = Enum.GetName(type, value).ToCamelCase();
				...
				stringBuilder.AppendLine("	public static function fromString(s:String):Null<Int> {");
				stringBuilder.AppendLine("		return switch (s) {");
			...
			foreach (var propertyInfo in enumNames)
			{
				if (IsPropertyUniqueToType(type, propertyInfo.Name))
				{
					if (propertyInfo.PropertyType.IsEnum)
					{
						...
						{
							var valName = Enum.GetName(propertyInfo.PropertyType, value).ToCamelCase();
							if (reservedWords.Contains(valName))
								valName = "_" + valName;
							if (valNames.ContainsKey(valName)) continue;
							var valValue = Convert.ToInt64(value);
